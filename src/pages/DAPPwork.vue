      ...
      <div class="css-work-tool-haw">
        <button class="css-3489" @click="toggleTheme()">
          <i
            class="pi"
            :class="{
              'pi-sun': theme_state[0] === 0,
              'pi-moon': theme_state[0] === 1,
              'pi-moon b': theme_state[0] === 2,
            }"
          ></i>
        </button>
      ...
      <div class="css-w-p-wpr">
        <div class="css-w-p-wrap">
          ...
          <ul class="css-w-p-ul" :class="{ active: category_visible[0] }">
            ...
            >
              <i
                v-if="scheme_progress.includes(category.id)"
                class="pi pi-check"
              ></i>
          ...
          <template v-if="item.answer === null">
            <div class="css-wq-pta">
              ...
              <template v-if="item.id === 10">
                <div class="css-wq-thl">
                  <div class="css-wq-txs txs">
                    <Chips
                      @click="helpers.sidebar = true"
                      @remove="chipRemove"
                      @add="chipAdded"
                      :allowDuplicate="false"
                      v-model="chart_data.labels"
                      separator=","
                    />

                    <Sidebar
                      v-model:visible="helpers.sidebar"
                      :modal="false"
                      position="right"
                    >
                      <Chart
                      ...
                      <span
                        >Total
                        {{
                          chart_data.datasets[0].data.length
                            ? chart_data.datasets[0].data.reduce(
                                (a, b) => a + b
                              )
                            : 0
                        }}</span
                      >
                      <template
                        v-for="(label, index) in chart_data.labels"
                        :key="index"
                      >
                        <div class="css-a278">
                          <label for="c8a9">
                            <span
                              >{{ label }} -
                              {{ chart_data.datasets[0].data[index] }} %</span
                            >
                          </label>
                          <input
                            id="c8a9"
                            class="css-628c"
                            v-model="chart_data.datasets[0].data[index]"
                            type="number"
                          />
                        </div>
                      </template>
...
export default {
  ...
  methods: {
    chipSetup() {
      this.chip_data = this.chart_data.labels;
    },
    chipAdded(e) {
      this.chip_data = e.value;
    },
    chipRemove(e) {
      const index = this.chip_data.indexOf(e.value[0]);

      const data = this.chart_data.datasets[0].data;

      this.chart_data.datasets[0].data = data.filter((e, i) => i !== index);

      this.chip_data = this.chip_data.filter((e, i) => i !== index);
    },
    toggleTheme() {
      this.theme_state[0] < 2
        ? this.theme_state[0]++
        : (this.theme_state[0] = 0);

      localStorage.setItem("theme", this.theme_state[0]);

      document.documentElement.className = this.themes[this.theme_state[0]];
    ...
    themeConfig() {
      ...
      if (this.theme_state[0] === 1 || this.theme_state[0] === 2)
        new_color = "#ffffff";

      this.lightOptions.plugins.legend.labels.color = new_color;
      this.chart_data.labels.push("empty");
    ...
    async pushNewReport() {
      ...
      this.audit_info["nid"] = Math.floor(Math.random() * 20000);
